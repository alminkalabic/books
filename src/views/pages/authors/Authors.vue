<template>
  <v-card class="elevation-0">
    <v-card-title>
      <v-row>
        <v-col cols="12" sm="12" md="6">
          <h2>{{ this.$route.meta.title }}</h2>
        </v-col>

        <v-col
          cols="12"
          sm="12"
          :md="userId === 1 && $route.path == '/authors' ? '4' : '6'"
        >
          <v-text-field
            v-model="search"
            append-icon="mdi-magnify"
            label="Search"
            single-line
            hide-details
            class="mt-0 pt-0"
          ></v-text-field>
        </v-col>

        <v-col cols="12" sm="12" md="2">
          <v-btn
            tile
            depressed
            color="primary"
            @click="addAuthor"
            class="w-100"
            v-bind:class="[userId === 1 ? 'mx-1' : 'd-none']"
          >
            Add author
          </v-btn>
        </v-col>
      </v-row>
    </v-card-title>

    <v-dialog v-model="editDialog" max-width="500px">
      <edit-author
        :authordata="editData"
        @changeEditDialog="changeEditDialog"
      />
    </v-dialog>

    <v-dialog v-model="deleteDialog" max-width="500px">
      <delete-author
        :authordata="deleteData"
        @changeDeleteDialog="changeDeleteDialog"
      />
    </v-dialog>

    <v-dialog v-model="addDialog" max-width="500px">
      <add-author @changeAddDialog="changeAddDialog" />
    </v-dialog>

    <v-data-table
      :headers="headers"
      :items="this.$store.getters.getAuthors"
      :loading="loading"
      :search="search"
    >
      <template v-slot:[`item.book`]="{ item }">
        <v-icon small class="mx-1" @click="viewBook(item.idBook)">
          mdi-eye
        </v-icon>
      </template>
      <template v-slot:[`item.actions`]="{ item }">
        <v-icon small class="mx-1" @click="viewAuthor(item.id)">
          mdi-eye
        </v-icon>
        <v-icon
          small
          @click="editAuthor(item)"
          v-bind:class="[userId === 1 ? 'mx-1' : 'd-none']"
        >
          mdi-pencil
        </v-icon>
        <v-icon
          small
          class="mx-1"
          @click="deleteAuthor(item)"
          v-bind:class="[userId === 1 ? 'mx-1' : 'd-none']"
        >
          mdi-delete
        </v-icon>
      </template>
    </v-data-table>
  </v-card>
</template>

<script>
import { readDataFromAPI } from "../../../api/global";
import EditAuthor from "./EditAuthor";
import DeleteAuthor from "./DeleteAuthor";
import AddAuthor from "./AddAuthor";
import store from "../../../store/index";

export default {
  data() {
    return {
      editDialog: false,
      editData: [],

      deleteDialog: false,
      deleteData: [],

      addDialog: false,

      userId: null,
      search: "",
      data: [],
      loading: true,
      headers: [
        { text: "First name", value: "firstName" },
        { text: "Last name", value: "lastName" },
        { text: "Book", value: "book", sortable: false },
        { text: "Actions", value: "actions", sortable: false },
      ],
    };
  },
  components: {
    EditAuthor,
    DeleteAuthor,
    AddAuthor,
  },
  methods: {
    readData() {
      this.loading = true;
      readDataFromAPI("Authors").then((response) => {
        this.loading = false;
        //this.data = response.data; - save data into global state instead state
        store.commit("setAuthors", response.data);
      });
    },
    viewBook(item) {
      this.$router.push(`books/${item}`);
    },
    viewAuthor(item) {
      this.$router.push(`authors/${item}`);
    },

    changeEditDialog(state) {
      this.editDialog = state;
    },

    changeDeleteDialog(state) {
      this.deleteDialog = state;
    },

    changeAddDialog(state) {
      this.addDialog = state;
    },

    editAuthor(item) {
      this.editDialog = true;
      this.editData = item;
    },

    deleteAuthor(item) {
      this.deleteDialog = true;
      this.deleteData = item;
    },

    addAuthor() {
      this.addDialog = true;
    },
  },
  mounted() {
    if (this.$store.getters.getUserInfo) {
      this.userId = this.$store.getters.getUserInfo.id;
    }

    this.readData();
  },
};
</script>
